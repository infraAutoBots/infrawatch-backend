...
# Pool global de engines SNMP (reutilizaÃ§Ã£o)
...
ESSENTIAL_OIDS = {
    ...
    "cpu": "1.3.6.1.4.1.2021.11.9.0"  # Apenas um OID por mÃ©trica
}


def print_logs(result: HostStatus):
...
class OptimizedMonitor:
    def __init__(self, session: Session = init_session()):
        ...
        # criar uma sessÃ£o e pegar todos os ips/host e InicializaÃ§Ã£o dos hosts
        if session:
    ...
    async def check_hosts_db(self):
        ...
        try:
            ...
            # Remove hosts que nÃ£o existem mais
    ...
    async def fast_ping_check(self, ips: List[str]) -> Dict[str, Tuple[bool, float]]:
        """Ping ultra-rÃ¡pido com timeout agressivo"""
        try:
    ...
    async def fast_tcp_check(self, ip: str) -> bool:
        """TCP check paralelo e ultra-rÃ¡pido"""
    ...
    async def check_single_host(self, host_status: HostStatus) -> HostStatus:
        """VerificaÃ§Ã£o completa de um host com fallbacks inteligentes"""
        ip = host_status.ip
        
        # 1. Ping primeiro (mais rÃ¡pido)
    ...
    async def run_monitoring(self, interval: float = 2.0):
        ...
        logger.info("ðŸš€ Iniciando monitoramento otimizado...")

        session_factory = init_session  # FunÃ§Ã£o que retorna uma nova Session
        ...
        while True:
            ...
            await check_task  # Garante que a checagem terminou antes do prÃ³ximo ciclo
...
# VersÃ£o ainda mais otimizada para casos extremos
class HyperFastMonitor(OptimizedMonitor):
    """VersÃ£o para quando vocÃª precisa de velocidade mÃ¡xima"""

    async def monitoring_cycle(self):
        await self.hyper_monitoring_cycle()

    async def hyper_check(self, ip: str) -> Tuple[bool, Optional[str]]:
        """VerificaÃ§Ã£o hÃ­brida ultra-rÃ¡pida"""
    ...
    async def hyper_monitoring_cycle(self):
        ...
        async with self.lock:
            for i, result in enumerate(results):
                ...
                status = "ðŸŸ¢" if is_alive else "ðŸ”´"
                snmp = "ðŸ“Š" if snmp_desc else "âŒ"
